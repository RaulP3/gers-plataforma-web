...
const databaseReady = new Promise((resolve, reject) => db.serialize(() => {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS samsara_mantenimiento (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  db.run('CREATE INDEX IF NOT EXISTS idx_samsara_mantenimiento_estado ON samsara_mantenimiento(estado)', [], () => {});
...
// ============ MANTENIMIENTO SAMSARA (DVIR defects + Issues) ============

function samsaraMaintHeaders() {
  return {
    'Authorization': `Bearer ${process.env.SAMSARA_API_TOKEN}`,
    'Content-Type': 'application/json',
  };
...
async function fetchSamsaraDefects(startIso, endIso) {
  const defects = [];
  let after = '';
  do {
    const params = { startTime: startIso, endTime: endIso, limit: 512 };
    if (after) params.after = after;
    const res = await axios.get(`${SAMSARA_API_BASE_URL}/fleet/defects/history`, { headers: samsaraMaintHeaders(), params, timeout: 15000 });
    const body = res.data || {};
    defects.push(...(Array.isArray(body.data) ? body.data : []));
...
async function fetchSamsaraIssues(startIso) {
  const issues = [];
  let after = '';
  do {
    const params = { startTime: startIso };
    if (after) params.after = after;
    const res = await axios.get(`${SAMSARA_API_BASE_URL}/issues/stream`, { headers: samsaraMaintHeaders(), params, timeout: 15000 });
    const body = res.data || {};
    issues.push(...(Array.isArray(body.data) ? body.data : []));
    after = body.pagination?.endCursor || '';
    if (!body.pagination?.hasNextPage || !after) break;
  } while (true);
  return issues;
...
let samsaraMantenimientoSyncInFlight = null;

async function performSamsaraMantenimientoSync() {
  if (!process.env.SAMSARA_API_TOKEN) return { skipped: true, reason: 'Sin token Samsara' };
  const fin = new Date();
  const inicio = new Date(fin.getTime() - 30 * 24 * 60 * 60 * 1000);
  const startIso = inicio.toISOString();
  const endIso = fin.toISOString();

  const [defects, issues] = await Promise.all([
    fetchSamsaraDefects(startIso, endIso),
    fetchSamsaraIssues(startIso),
  ]);

  const upsert = async (item) => {
    const existing = await getQuery('SELECT id, estado FROM samsara_mantenimiento WHERE samsara_id = ?', [item.samsara_id]);
    if (existing) {
      await runQuery(
        `UPDATE samsara_mantenimiento SET vehicle_id = ?, vehicle_name = ?, entidad = ?, descripcion = ?, severidad = ?, estado = ?, fecha_creado = ?, fecha_resuelto = ?, creado_por = ?, updated_at = datetime('now') WHERE id = ?`,
        [item.vehicle_id, item.vehicle_name, item.entidad, item.descripcion, item.severidad, item.estado, item.fecha_creado, item.fecha_resuelto, item.creado_por, existing.id]
      );
      return { nuevo: false };
    }
    await runQuery(
      `INSERT INTO samsara_mantenimiento (samsara_id, fuente, vehicle_id, vehicle_name, entidad, descripcion, severidad, estado, fecha_creado, fecha_resuelto, creado_por) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [item.samsara_id, item.fuente, item.vehicle_id, item.vehicle_name, item.entidad, item.descripcion, item.severidad, item.estado, item.fecha_creado, item.fecha_resuelto, item.creado_por]
    );
    return { nuevo: true };
  };
  ...
  for (const d of defects || []) {
    ...
    const nombre = vehicle.name || trailer.name || 'Vehículo';
    const resuelto = Boolean(d.isResolved);
    const tipoDefecto = d.defectType || 'Defecto DVIR';
    const { nuevo } = await upsert({
      samsara_id: `dvir-${d.id}`,
      fuente: 'dvir',
      vehicle_id: vehicle.id || trailer.id || '',
      vehicle_name: nombre,
      entidad: tipoDefecto,
      descripcion: d.comment || d.mechanicNotes || '',
      severidad: 'media',
      estado: resuelto ? 'resuelto' : 'abierto',
      fecha_creado: d.createdAtTime || '',
      fecha_resuelto: resuelto ? (d.resolvedAtTime || '') : '',
      creado_por: d.resolvedBy?.name || '',
    });
    ...
    if (!resuelto) {
      ...
      if (!recent) {
        await createAlertRecord({
          ...
          vehicle_name: nombre,
          tipo: 'mantenimiento',
          mensaje: `DVIR Samsara: ${nombre} reporta "${tipoDefecto}"${d.comment ? ` — ${d.comment}` : ''}`,
          severidad: 'media',
        });
        nuevosAlertas += 1;
      }
  ...
  for (const i of issues || []) {
    const asset = i.asset || {};
    const nombre = asset.name || (asset.id ? `Activo ${asset.id}` : 'Activo');
    const estado = i.status === 'resolved' ? 'resuelto' : i.status === 'dismissed' ? 'descartado' : i.status === 'inProgress' ? 'en_progreso' : 'abierto';
    const severidad = i.priority === 'high' ? 'alta' : i.priority === 'medium' ? 'media' : 'baja';
    const titulo = i.title || 'Issue';
    const { nuevo } = await upsert({
      samsara_id: `issue-${i.id}`,
      fuente: 'issue',
      vehicle_id: asset.id || '',
      vehicle_name: nombre,
      entidad: titulo,
      ...
      severidad,
      estado,
      fecha_creado: i.submittedAtTime || i.createdAtTime || '',
      fecha_resuelto: estado === 'resuelto' ? (i.updatedAtTime || '') : '',
      creado_por: i.assignedTo?.id ? `Asignado ${i.assignedTo.id}` : '',
    });
    if (nuevo) insertados += 1;
    if (estado === 'abierto' || estado === 'en_progreso') {
      const recent = await getQuery(
        `SELECT id FROM alertas WHERE tipo = 'mantenimiento' AND mensaje LIKE ? AND timestamp > datetime('now', '-24 hours')`,
        [`%${nombre}%${titulo}%`]
      );
      if (!recent) {
        await createAlertRecord({
          ...
          vehicle_name: nombre,
          tipo: 'mantenimiento',
          mensaje: `Issue Samsara [${severidad === 'alta' ? 'ALTA' : severidad === 'media' ? 'MEDIA' : 'BAJA'}]: ${titulo}${i.description ? ` — ${i.description}` : ''}`,
          severidad,
        });
        nuevosAlertas += 1;
      }
  ...
  return { checked: defects.length + issues.length, defectos: defects.length, issues: issues.length, insertados, nuevosAlertas };
}

function checkSamsaraMantenimiento() {
  if (!samsaraMantenimientoSyncInFlight) {
    samsaraMantenimientoSyncInFlight = performSamsaraMantenimientoSync().finally(() => { samsaraMantenimientoSyncInFlight = null; });
  }
  return samsaraMantenimientoSyncInFlight;
}

app.get('/api/samsara/mantenimiento', async (req, res) => {
  try {
    const rows = await allQuery('SELECT * FROM samsara_mantenimiento ORDER BY fecha_creado DESC, id DESC LIMIT 300');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/samsara/mantenimiento/sync', async (req, res) => {
  try {
    res.json(await checkSamsaraMantenimiento());
  } catch (error) {
    console.error('Error sync Samsara mantenimiento:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
const runLiveSync = async () => {
  ...
  liveSyncInFlight = (async () => {
    ...
    try {
      await checkSamsaraMantenimiento();
    } catch (samsaraMaintErr) {
      console.error('Error sincronizando mantenimiento Samsara:', samsaraMaintErr.message);
    }
